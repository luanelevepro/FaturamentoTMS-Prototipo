                        ...
                        <div className="space-y-4">
                            ...
                            {sortedLoads.map(load => (
                                <LoadCard
                                    key={load.id}
                                    load={load}
                                    onViewDetails={() => handleViewDetails(`Carga: ${load.clientName}`, { type: 'LOAD_DETAIL', ...load })}
                                    onSchedule={() => handleOpenSchedule(load)}
                                />
                            ))}
                        </div>
                    </Column>
                    ...
                    <TripColumn
                        ...
                        actionButton={(trip) => (
                            <button
                                ...
                                className={`flex items-center gap-2 px-6 py-2.5 text-white text-xs font-black rounded-xl shadow-lg uppercase tracking-wide transition-all ${!isTripFiscalOk(trip) ? 'bg-gray-300 cursor-not-allowed grayscale' : 'bg-black hover:bg-gray-800'
                                    }`}
...
const TripColumn: React.FC<TripColumnProps> = ({ title, trips, headerColor, accentColor, statusColor, statusLabel, showProgress, isCompleted, onViewDetails, actionButton, tooltip, onEmitFiscal }) => (
    <Column title={title} count={trips.length} headerColor={headerColor} accentColor={accentColor} tooltip={tooltip}>
        <div className="space-y-5">
            ...
            {trips.map(trip => (
                <TripCard key={trip.id} trip={trip} statusColor={statusColor} statusLabel={statusLabel} showProgress={showProgress} isCompleted={isCompleted} onViewDetails={onViewDetails} actionButton={actionButton} accentColor={accentColor} onEmitFiscal={onEmitFiscal} />
            ))}
        </div>
    </Column>
);
...
const TripCard: React.FC<TripCardProps> = ({ trip, statusColor, statusLabel, showProgress, isCompleted, onViewDetails, actionButton, accentColor, onEmitFiscal }) => {
    ...
    return (
        ...
        >
            ...
            <div className="flex justify-between items-start mb-3">
                ...
                {trip.mdfe && (
                    <span className="text-[10px] font-black px-2 py-1 bg-green-100 text-green-700 border border-green-200 rounded-md uppercase flex items-center gap-1">
                        <CheckCircle size={10} /> MDF-e OK
                    </span>
                )}
            </div>
            ...
            {/* MDF-e info shown in header if authorized */}
