...
import { validateSegmentCompatibility, validateAddLoadToTrip, validateEmitCTe, validateEmitMDFe } from './lib/validations';
...
export default function App() {
  ...
  // const [view, setView] = useState<ViewState>('LIST_V2'); // Deprecated, derived from URL

  const location = useLocation();
  const getViewFromPath = (path: string): ViewState => {
    if (path.includes('/viagens/lista')) return 'LIST';
    if (path.includes('/viagens/cronograma')) return 'TIMELINE';
    if (path.includes('/viagens/board')) return 'LIST_V2';
    return 'LIST_V2'; // Default
  };
  const view = getViewFromPath(location.pathname);
  const setView = (v: ViewState) => {
    // No-op or navigate? For now no-op as sidebar drives nav. 
    // Ideally we would navigate() here but TripShell calls onChangeView.
    // We can leave it as no-op or implement navigation if TripShell has internal nav buttons (unlikely for main views).
    console.log('Request to change view to', v);
  };
  ...
  const handleUpdateTripStatus = (tripId: string, newStatus: Trip['status'], pod?: string) => {
    setTrips(trips.map(t => {
      if (t.id === tripId) {
        ...
        // If completing the trip, also mark all its deliveries as Delivered with deliveryDate
        if (newStatus === 'Completed') {
          ...
          updatedTrip.legs = updatedTrip.legs.map(l => ({
            ...
            deliveries: l.deliveries.map(d => ({ 
              ...d, 
              status: 'Delivered',
              deliveryDate: d.deliveryDate || now // Usar deliveryDate existente ou definir como agora
            }))
          }));
        }
        return updatedTrip;
  ...
  // Handler para "emitir" CT-e e MDF-e de uma carga específica
  const handleEmitFiscal = (loadId: string) => {
    ...
    if (!trip) {
      alert("❌ ERRO: Carga não vinculada a nenhuma viagem.");
      return;
    }

    // Validação simplificada para o protótipo: requer que a carga esteja "Scheduled" ou "Emitted"
    if (load.status !== 'Scheduled' && load.status !== 'Emitted') {
      alert("❌ BLOQUEIO: Status da carga inválido para emissão.");
      return;
    }

    // 1. Gera CT-e se necessário
    let updatedCte = load.cte;
    if (!updatedCte || updatedCte.status !== 'Authorized') {
      updatedCte = {
        id: `cte-${Date.now()}`,
        loadId: load.id,
        ...
        freightValue: load.weight ? load.weight * 2.5 : 1500,
    ...
    const mockMDFe = {
      id: `mdfe-${Date.now()}`,
      tripId: trip.id,
      number: `MDF${String(Math.floor(Math.random() * 1000000)).padStart(6, '0')}`,
      accessKey: `432${new Date().getFullYear()}${String(Math.floor(Math.random() * 100000000000000000000000000000000000000000)).padStart(44, '0')}`,
      status: 'Authorized' as const,
      emissionDate: new Date().toISOString(),
      authorizationDate: new Date().toISOString(),
      createdAt: new Date().toISOString()
    ...
    setLoads(loads.map(updateLoad));

    setTrips(trips.map(t => {
      if (t.id === trip.id) {
        const updatedLoads = t.loads?.map(updateLoad);
        const currentMdfes = t.mdfes || [];
        return {
          ...t,
          loads: updatedLoads,
          mdfes: [...currentMdfes, mockMDFe]
        };
      }
      return t;
    }));
  ...
  return (
    <SystemLayout>
      ...
      <TripShell
        ...
        onEmitFiscal={handleEmitFiscal}
        heightMode="container" // Use container height since we are inside layout
        className="rounded-lg border shadow-sm" // Add some styling
      />
    </SystemLayout>
  );
}
