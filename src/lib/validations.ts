...
/**
 * Validação para Emitir MDF-e (Per-Load)
 * Hard Block: Requer que a carga associada tenha CT-e autorizado
 */
export function validateEmitMDFe(
  load: Load,
  trip: Trip | null
): ValidationResult {
  if (!trip) {
    return { valid: false, type: 'hard_block', error: 'MDF-e requer uma viagem vinculada.' };
  }
  ...
  if (!load.cte || load.cte.status !== 'Authorized') {
    return {
      valid: false,
      type: 'hard_block',
      error: 'MDF-e bloqueado: A carga deve possuir CT-e autorizado antes de manifestar.'
    };
  }

  // Verifica se já manifestado
  if (load.mdfe && load.mdfe.status === 'Authorized') {
    return {
      valid: false,
      type: 'hard_block',
      error: 'MDF-e já autorizado para esta carga.'
    };
  }
